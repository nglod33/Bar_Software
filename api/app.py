...
if not engine.dialect.has_table(engine, "Swipes"):
...
def post_picture():
    ...
    if file and allowed_file(file.filename):
        ...
        results_dict = imageReader.readLicense(os.path.join(app.config['UPLOAD_FOLDER'], filename))
...
def get_picture():
    ...
    if file and allowed_file(file.filename):
        ...
        file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
        results_dict = imageReader.readLicense(os.path.join(app.config['UPLOAD_FOLDER'], filename))
        timestamps = checkHash(results_dict['license_id'], request.form.get("token"))
...
def checkHash(IDString, token):
    ...
    for result in results:
        timestamp_results.append(result.ts)
